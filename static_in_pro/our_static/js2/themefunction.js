jQuery(document).on('ready', function() {
	...
	var _tg_categoryAlbumbs = jQuery('' +
		...
		'[id="bt-weddingalbum"],' +
		'[id="bt-kidsalbum"],' +
		' [id="bt-twinssisteralbum"],' +
		' [id="bt-halloweenalbum"],' +
		' [id="bt-stationphotographyalbum"],' +
		' [id="bt-femalemodelalbum"],' +
		' [id="bt-femalemodeltwoalbum"],' +
		' [id="bt-randomclickalbum"]' +
		'');
	...
	/* -------------------------------------
			CUBE GALLERY
	-------------------------------------- */
	if (jQuery('#galleryGeneral').length > 0){
		var gallerySpace = jQuery('#galleryGeneral').attr('data-columnspace');
		var gallerycol = jQuery('#galleryGeneral').attr('data-gallerycolumns');
		jQuery('#galleryGeneral').cubeportfolio({
			filters: '#js-filters-agency',
			layoutMode: 'grid',
			sortToPreventGaps: true,
			animationType: 'fadeOutTop',
			gapHorizontal: parseInt(gallerySpace),
			gapVertical: parseInt(gallerySpace),
			gridAdjustment: 'responsive',
			mediaQueries: [
				{
					width: 768,
					cols: parseInt(gallerycol),
				},
				{
					width: 480,
					cols: 2,
